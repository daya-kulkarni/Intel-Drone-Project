...
############################################################################
    ...
    #    if not tello.takeoff():
        ...
        startTime = time()
        while(True):
            print("tello flight time")
            #print(tello.get_flight_time())

            #test1=tello.get_flight_time().strip('s')
            #remove the 0s s at the end of the characters
#time to end the flight
        #    print("test value is")
            #print(test1)

        #    test = int('test1')
        #    if (tello.get_flight_time() and (test > 10)):

            #    print("Three minutes of flight time have been logged - triggering autolanding")
            #    tello.land()
            ...
            #Adding capability to flip once it detects a person
...
#person not detected.. classification id 80
            if int(det_label) == 0 and detect_gate:
                ...
                #Pseudo: get time and make sure we don't flip too often (20 seconds)
            # Draw performance stats over frame
            inf_time_message = "Inference time: N\A for async mode" if is_async_mode else \
                "Inference time: {:.3f} ms".format(det_time * 1e3)
            render_time_message = "OpenCV rendering time: {:.3f} ms".format(render_time * 1e3)
