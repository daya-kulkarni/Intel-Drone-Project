...
############################################################################
        ...
        #copied from above - preps video feed and counts frames
        footage = tello.get_frame_read()
    ######3    footage = tello.get_video_capture()
        number_input_frames = int(footage.get(cv2.CAP_PROP_FRAME_COUNT))
        number_input_frames = 1 if number_input_frames != -1 and number_input_frames < 0 else number_input_frames

        if number_input_frames != 1:
            #ret, frame = footage.read()
            ret, frame = footage.update_frame()
        else:
            is_async_mode = False
            wait_key_code = 0
...
#move to only infernce mutlipletime
        while(True):

            if is_async_mode:
                ret = True #footage.frame
                #next_frame = footage.frame
                #next_frame = footage.read() # In cap.read its 91600 size
                next_frame = footage.update_frame()
            else:
                ret = True #footage.frame
                #frame = footage.frame
                #frame = footage.read()
                frame = footage.update_frame()
                # print ("footage.frame type is: " + str(type(footage.drone)))
                #print("Output Representation: " + str(footage.frame))
                print("Output Representation: " + str(footage.update_frame()))
                #print("Here's the size of the array: " + str(footage.frame.size))
                #print("Here's the size of the array: " + str(footage.read().size()))
